...
function [Elite_antlion_position,Elite_antlion_fitness]=ALO(N,Max_iter,lb,ub,dim,f,signal)
...
v(j)=signal(m);
    ...
    antlions_fitness(1,i)=fobj(u,v,size(u,2)); 
end
...
for i=1:size(ant_position,1)
      ...
      ant_position(i,:)= 0.1*(RA+RE)/2; % Equation (2.13) in the paper       
   %ant_position=ant_position.*9000;
    ...
    for i=1:size(ant_position,1)  
        ...
        ants_fitness(1,i)=fobj(u,v,size(u,2));        
    ...
    Elite_antlion_position=Elite_antlion_position;
